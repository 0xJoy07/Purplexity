...
export default function SignInPage() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.fromTo(
        ...
        { opacity: 1, y: 0, scale: 1, duration: 0.8, ease: 'back.out(1.7)', delay: 0.1 }
      );
      gsap.fromTo(
        cardRef.current,
        { opacity: 0, y: 40, scale: 0.95 },
        { opacity: 1, y: 0, scale: 1, duration: 0.7, ease: 'power3.out', delay: 0.25 }
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      gsap.to(cardRef.current, { opacity: 0, y: -20, scale: 1.02, duration: 0.4, ease: 'power2.in' });
    ...
    <div ref={containerRef} className="auth-page-root">
      ...
      <div ref={cardRef} className="auth-card">
        <div className="auth-card-header">
          ...
          <p className="auth-card-desc">Sign in to continue your research</p>
        </div>
        ...
        <p className="auth-field auth-footer-text">
          No account?{' '}
          <Link href="/signup" className="auth-link font-semibold">Create one free</Link>
        </p>
      </div>
    </div>
  );
