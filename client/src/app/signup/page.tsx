...
import { CheckCircle2, Eye, EyeOff, Loader2, Sparkles, User } from 'lucide-react';
...
function FloatingOrbs() {
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden">
      <div className="auth-orb auth-orb-1" style={{ animationDelay: '-3s' }} />
      <div className="auth-orb auth-orb-2" style={{ animationDelay: '-6s' }} />
      <div className="auth-orb auth-orb-3" style={{ animationDelay: '-1s' }} />
  ...
  return (
    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="mt-2 space-y-1.5">
      <div className="flex gap-1">
        {[1, 2, 3].map(i => (
          <div key={i} className={`h-1 flex-1 rounded-full transition-colors duration-300 ${i <= strength ? colors[strength] : 'bg-border'}`} />
        ))}
      </div>
      <div className="flex gap-3">
        {checks.map(c => (
          <span key={c.label} className={`text-[10px] transition-colors ${c.ok ? 'text-accent' : 'text-text-subtle'}`}>
            {c.ok ? '✓' : '○'} {c.label}
          </span>
        ))}
      </div>
...
export default function SignUpPage() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo('.auth-field', { opacity: 0, x: -20 }, { opacity: 1, x: 0, duration: 0.5, ease: 'power2.out', stagger: 0.07, delay: 0.45 });
    }, containerRef);
    return () => ctx.revert();
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      gsap.to('.auth-form-wrap', { opacity: 0.4, duration: 0.3 });
    } catch (err: any) {
    ...
    <div ref={containerRef} className="auth-page-root">
      ...
      <div ref={cardRef} className="auth-card">
        <div className="auth-card-header">
          ...
          <p className="auth-card-desc">100,000 tokens free. No credit card needed.</p>
        </div>

        {success ? (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="auth-success">
            <CheckCircle2 className="mx-auto mb-3 h-12 w-12 text-accent" />
            <p className="text-center text-sm font-medium text-foreground">{success}</p>
            <p className="mt-1 text-center text-xs text-text-subtle">Check your inbox and click the verification link.</p>
            <Link href="/signin" className="auth-submit-btn mt-4 text-center no-underline">Go to Sign In</Link>
          </motion.div>
        ) : (
          <>
            <div className="auth-field grid grid-cols-2 gap-3">
              <button type="button" onClick={loginWithGoogle} className="auth-oauth-btn group">
                <GoogleIcon className="h-4 w-4 transition-transform group-hover:scale-110" />
                <span>Google</span>
              </button>
              <button type="button" onClick={loginWithGithub} className="auth-oauth-btn group">
            ...
            <form onSubmit={handleSubmit} className="auth-form-wrap space-y-4">
              <div className="auth-field space-y-1.5">
                <label className="auth-label">Full name</label>
                <div className="relative">
                  <input type="text" required value={name} onChange={e => setName(e.target.value)} placeholder="John Doe" className="auth-input pl-9" autoComplete="name" />
                  <User className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-text-subtle" />
                </div>
              </div>
              ...
              <div className="auth-field space-y-1.5">
                ...
                <div className="relative">
                  <input type={showPass ? 'text' : 'password'} required value={password} onChange={e => setPassword(e.target.value)} placeholder="Min. 8 characters" className="auth-input pr-10" autoComplete="new-password" />
        ...
        <p className="auth-field auth-footer-text">
