...
export function Sidebar({ isOpen, onToggle, activeConversationId, onSelectConversation, onNewChat, refreshTrigger }: SidebarProps) {
  ...
  const fetchConversations = useCallback(async () => {
    try {
      setIsLoading(true);
      if (user && token) {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE}/conversations/user/${user.id}`, {
  ...
  const handleDelete = async (event: React.MouseEvent, conversationId: string) => {
    ...
    try {
      ...
      if (user && token) {
        await fetch(`${process.env.NEXT_PUBLIC_API_BASE}/conversations/${conversationId}`, {
          method: 'DELETE',
          headers: { Authorization: `Bearer ${token}` },
        });
      } else {
